 ...
 *
 * In particular, the <code>ensureCapacity()</code>, <code>grow()</code>, <code>trim()</code> and <code>setLength()</code> methods allow to handle arrays much like array lists. This can be very useful
 * when efficiency (or syntactic simplicity) reasons make array lists unsuitable.
 *
 * <P><strong>Warning:</strong> if your array is not of type {@code Object[]}, {@link #ensureCapacity(Object[],int,int)} and {@link #grow(Object[],int,int)} will use
 * {@linkplain java.lang.reflect.Array#newInstance(Class,int) reflection} to preserve your array type. Reflection is <em>significantly slower</em> than using <code>new</code>. This phenomenon is
 * particularly evident in the first growth phases of an array reallocated with doubling (or similar) logic.
 *
 * <h2>Sorting</h2>
 *
 ...
 * quicksort). Several algorithms provide a parallel version, that will use the {@linkplain Runtime#availableProcessors() number of cores available}.
 *
...
public class ObjectArrays {
	 ...
	 *
	 * <P>This method returns a new array of the given length whose element are of the same class as of those of <code>prototype</code>. In case of an empty array, it tries to return
	 * {@link #EMPTY_ARRAY}, if possible.
	 *
	...
	public static <K> boolean equals( final K[] a1, final K a2[] ) {
		...
		while ( i-- != 0 )
			if ( !( ( a1[ i ] ) == null ? ( a2[ i ] ) == null : ( a1[ i ] ).equals( a2[ i ] ) ) ) return false;
		return true;
	}
	...
	private static <K> void selectionSort( final K[] a, final int from, final int to ) {
		for ( int i = from; i < to - 1; i++ ) {
			int m = i;
			for ( int j = i + 1; j < to; j++ )
				if ( ( ( (Comparable<K>)( a[ j ] ) ).compareTo( a[ m ] ) < 0 ) ) m = j;
			if ( m != i ) {
				final K u = a[ i ];
				a[ i ] = a[ m ];
				a[ m ] = u;

 ...
 *
 * <p>In particular, the <code>ensureCapacity()</code>, <code>grow()</code>, <code>trim()</code> and <code>setLength()</code> methods allow to handle big arrays much like array lists.
 *
 * <P>Note that it.unimi.dsi.fastutil.io.BinIO and it.unimi.dsi.fastutil.io.TextIO contain several methods that make it possible to load and save big arrays of primitive types as
 * sequences of elements in {@link java.io.DataInput} format (i.e., not as objects) or as sequences of lines of text.
 *
 * @see BigArrays */
public class IntBigArrays {
	...
	/** Copies a big array from the specified source big array, beginning at the specified position, to the specified position of the destination big array. Handles correctly overlapping regions of the
	 * same big array.
	 * 
	...
	public static int[][] newBigArray( final long length ) {
		...
		else for ( int i = 0; i < baseLength; i++ )
			base[ i ] = new int[ SEGMENT_SIZE ];
		return base;
	}
	 ...
	 *
	 * <P>If you cannot foresee whether this big array will need again to be enlarged, you should probably use <code>grow()</code> instead.
	 *
	 * <p><strong>Warning:</strong> the returned array might use part of the segments of the original array, which must be considered read-only after calling this method.
	 *
	 * @param array a big array.
	 * @param length the new minimum length for this big array.
	 * @return <code>array</code>, if it contains <code>length</code> entries or more; otherwise, a big array with <code>length</code> entries whose first <code>length(array)</code> entries are the
	 * same as those of <code>array</code>. */
	 ...
	 * @return <code>array</code>, if it can contain <code>length</code> entries or more; otherwise, a big array with <code>length</code> entries whose first <code>preserve</code> entries are the same
	...
	public static int[][] ensureCapacity( final int[][] array, final long length, final long preserve ) {
		...
		if ( length > oldLength ) {
			...
			else for ( int i = valid; i < baseLength; i++ )
				base[ i ] = new int[ SEGMENT_SIZE ];
	 ...
	 *
	 * <P>If you want complete control on the big array growth, you should probably use <code>ensureCapacity()</code> instead.
	 *
	 * <p><strong>Warning:</strong> the returned array might use part of the segments of the original array, which must be considered read-only after calling this method.
	 *
	 * @param array a big array.
	 * @param length the new minimum length for this big array.
	 * @return <code>array</code>, if it can contain <code>length</code> entries; otherwise, a big array with max(<code>length</code>,<code>length(array)</code>/&phi;) entries whose first
	 ...
	 * @return <code>array</code>, if it contains <code>length</code> entries or less; otherwise, a big array with <code>length</code> entries whose entries are the same as the first
	 * <code>length</code> entries of <code>array</code>. */
	 ...
	 * @return <code>array</code>, if it contains exactly <code>length</code> entries; otherwise, if it contains <em>more</em> than <code>length</code> entries, a big array with <code>length</code>
	 * entries whose entries are the same as the first <code>length</code> entries of <code>array</code>; otherwise, a big array with <code>length</code> entries whose first <code>length(array)</code>
	 ...
	 *
	 * <P>This method uses a backward loop. It is significantly faster than the corresponding method in {@link java.util.Arrays}.
	 *
	 * @param array a big array.
	 ...
	 *
	 * <P>If possible (i.e., <code>from</code> is 0) this method uses a backward loop. In this case, it is significantly faster than the corresponding method in {@link java.util.Arrays}.
	 *
	 * @param array a big array.
	 ...
	 *
	 * The string representation consists of a list of the big array's elements, enclosed in square brackets ("[]"). Adjacent elements are separated by the characters ", " (a comma followed by a
	 * space). Returns "null" if <code>a</code> is null.
	 *
	...
	/** A type-specific content-based hash strategy for big arrays.
	 *
	 * <P>This hash strategy may be used in custom hash collections whenever keys are big arrays, and they must be considered equal by content. This strategy will handle <code>null</code> correctly,
	...
	private static void selectionSort( final int[][] a, final long from, final long to, final IntComparator comp ) {
		for ( long i = from; i < to - 1; i++ ) {
			...
			for ( long j = i + 1; j < to; j++ )
				if ( comp.compare( IntBigArrays.get( a, j ), IntBigArrays.get( a, m ) ) < 0 ) m = j;
			if ( m != i ) swap( a, i, m );
		}
	}
	...
	private static void selectionSort( final int[][] a, final long from, final long to ) {
		for ( long i = from; i < to - 1; i++ ) {
			long m = i;
			for ( long j = i + 1; j < to; j++ )
				if ( ( ( IntBigArrays.get( a, j ) ) < ( IntBigArrays.get( a, m ) ) ) ) m = j;
			if ( m != i ) swap( a, i, m );
		}
	}
	...
	/** Searches a range of the specified big array for the specified value using the binary search algorithm. The range must be sorted prior to making this call. If it is not sorted, the results are
	 ...
	 * @return index of the search key, if it is contained in the big array; otherwise, <tt>(-(<i>insertion point</i>) - 1)</tt>. The <i>insertion point</i> is defined as the the point at which the
	 * value would be inserted into the big array: the index of the first element greater than the key, or the length of the big array, if all elements in the big array are less than the specified
	...
	/** Searches a big array for the specified value using the binary search algorithm. The range must be sorted prior to making this call. If it is not sorted, the results are undefined. If the range
	...
	/** Searches a range of the specified big array for the specified value using the binary search algorithm and a specified comparator. The range must be sorted following the comparator prior to
	 * making this call. If it is not sorted, the results are undefined. If the range contains multiple elements with the specified value, there is no guarantee which one will be found.
	 *
	 * @param a the big array to be searched.
	 * @param from the index of the first element (inclusive) to be searched.
	 * @param to the index of the last element (exclusive) to be searched.
	...
	/** Searches a big array for the specified value using the binary search algorithm and a specified comparator. The range must be sorted following the comparator prior to making this call. If it is
	 * not sorted, the results are undefined. If the range contains multiple elements with the specified value, there is no guarantee which one will be found.
	 *
	 * @param a the big array to be searched.
	 * @param key the value to be searched for.
	 * @param c a comparator.
	 ...
	 * 5&minus;27 (1993), and further improved using the digit-oracle idea described by Juha K&auml;rkk&auml;inen and Tommi Rantala in &ldquo;Engineering radix sort for strings&rdquo;, <i>String
	 * Processing and Information Retrieval, 15th International Symposium</i>, volume 5280 of Lecture Notes in Computer Science, pages 3&minus;14, Springer (2008).
	 *
	 * <p>This implementation is significantly faster than quicksort already at small sizes (say, more than 10000 elements), but it can only sort in ascending order. It will allocate a support array
	 * of bytes with the same number of elements as the array to be sorted.
	 * 
	...
	public static void radixSort( final int[][] a, final long from, final long to ) {
		...
		while ( offsetPos > 0 ) {
			...
			for ( long i = length; i-- != 0; )
				ByteBigArrays.set( digit, i, (byte)( ( ( ( IntBigArrays.get( a, first + i ) ) >>> shift ) & DIGIT_MASK ) ^ signMask ) );
			for ( long i = length; i-- != 0; )
	...
	private static void selectionSort( final int[][] a, final int[][] b, final long from, final long to ) {
		for ( long i = from; i < to - 1; i++ ) {
			long m = i;
			for ( long j = i + 1; j < to; j++ )
				if ( ( ( IntBigArrays.get( a, j ) ) < ( IntBigArrays.get( a, m ) ) ) || ( ( IntBigArrays.get( a, j ) ) == ( IntBigArrays.get( a, m ) ) )
						&& ( ( IntBigArrays.get( b, j ) ) < ( IntBigArrays.get( b, m ) ) ) ) m = j;
			if ( m != i ) {
	...
	/** Sorts the specified pair of big arrays lexicographically using radix sort. <p>The sorting algorithm is a tuned radix sort adapted from Peter M. McIlroy, Keith Bostic and M. Douglas McIlroy,
	 * &ldquo;Engineering radix sort&rdquo;, <i>Computing Systems</i>, 6(1), pages 5&minus;27 (1993), and further improved using the digit-oracle idea described by Juha K&auml;rkk&auml;inen and Tommi
	 * Rantala in &ldquo;Engineering radix sort for strings&rdquo;, <i>String Processing and Information Retrieval, 15th International Symposium</i>, volume 5280 of Lecture Notes in Computer Science,
	 * pages 3&minus;14, Springer (2008).
	 *
	 ...
	 *
	 * <p>This implementation is significantly faster than quicksort already at small sizes (say, more than 10000 elements), but it can only sort in ascending order. It will allocate a support array
	 * of bytes with the same number of elements as the arrays to be sorted.
	 * 
	...
	public static void radixSort( final int[][] a, final int[][] b, final long from, final long to ) {
		...
		while ( offsetPos > 0 ) {
			...
			// Count keys.
			for ( long i = length; i-- != 0; )
				ByteBigArrays.set( digit, i, (byte)( ( ( ( IntBigArrays.get( k, first + i ) ) >>> shift ) & DIGIT_MASK ) ^ signMask ) );
			for ( long i = length; i-- != 0; )
				count[ ByteBigArrays.get( digit, i ) & 0xFF ]++;
			// Compute cumulative distribution and push non-singleton keys on stack.
			int lastUsed = -1;

 ...
 *
 * <P>Instances of this class use as reference list a <em>reference array</em>, which must be provided to each constructor, and represent a priority queue using a backing array of integer
 * indices&mdash;all operations are performed directly on the array. The array is enlarged as needed, but it is never shrunk. Use the {@link #trim()} method to reduce its size, if necessary.
 *
 * <P>This implementation is extremely inefficient, but it is difficult to beat when the size of the queue is very small. Moreover, it allows to enqueue several time the same index, without
 * limitations. */
public class IntArrayIndirectPriorityQueue extends AbstractIndirectPriorityQueue<Integer> implements IntIndirectPriorityQueue {
	 ...
	 *
	 * <P>Note that for efficiency reasons this method will <em>not</em> throw an exception when <code>x</code> is already in the queue. However, the queue state will become inconsistent and the
	...
	/** {@inheritDoc}
	 *
	 * <P>Note that for efficiency reasons this method will <em>not</em> throw an exception when <code>index</code> is not in the queue. */

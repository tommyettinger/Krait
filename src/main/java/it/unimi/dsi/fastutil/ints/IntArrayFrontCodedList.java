 ...
 * 
 * <P>This class stores immutably a list of arrays in a single large array using front coding (of course, the compression will be reasonable only if the list is sorted lexicographically&mdash;see
 * below). It implements an immutable type-specific list that returns the <var>i</var>-th array when calling {@link #get(int) get(<var>i</var>)}. The returned array may be freely modified.
 *
 * <P>Front coding is based on the idea that if the <var>i</var>-th and the (<var>i</var>+1)-th array have a common prefix, we might store the length of the common prefix, and then the rest of the
 * second array.
 *
 * <P>This approach, of course, requires that once in a while an array is stored entirely. The <em>ratio</em> of a front-coded list defines how often this happens (once every {@link #ratio()} arrays).
 * A higher ratio means more compression, but means also a longer access time, as more arrays have to be probed to build the result. Note that we must build an array every time {@link #get(int)} is
 * called, but this class provides also methods that extract one of the stored arrays in a given array, reducing garbage collection. See the documentation of the family of <code>get()</code> methods.
 *
 * <P>By setting the ratio to 1 we actually disable front coding: however, we still have a data structure storing large list of arrays with a reduced overhead (just one integer per array, plus the
 * space required for lengths).
 *
 * <P>Note that the typical usage of front-coded lists is under the form of serialized objects; usually, the data that has to be compacted is processed offline, and the resulting structure is stored
 * permanently. Since the pointer array is not stored, the serialized format is very small.
 *
 * <H2>Implementation Details</H2>
 * 
 * <P>All arrays are stored in a {@linkplain it.unimi.dsi.fastutil.BigArrays big array}. A separate array of pointers indexes arrays whose position is a multiple of the ratio: thus, a higher ratio
 * means also less pointers.
 * 
 * <P>More in detail, an array whose position is a multiple of the ratio is stored as the array length, followed by the elements of the array. The array length is coded by a simple variable-length
 * list of <var>k</var>-1 bit blocks, where <var>k</var> is the number of bits of the underlying primitive type. All other arrays are stored as follows: let <code>common</code> the length of the
 * maximum common prefix between the array and its predecessor. Then we store the array length decremented by <code>common</code>, followed by <code>common</code>, followed by the array elements whose
 * index is greater than or equal to <code>common</code>. For instance, if we store <code>foo</code>, <code>foobar</code>, <code>football</code> and <code>fool</code> in a front-coded character-array
 * list with ratio 3, the character array will contain
 *
 * <pre> <b>3</b> f o o <b>3</b> <b>3</b> b a r <b>5</b> <b>3</b> t b a l l <b>4</b> f o o l </pre> */
public class IntArrayFrontCodedList extends AbstractObjectList<int[]> implements Serializable, Cloneable, RandomAccess {
		...
		while ( arrays.hasNext() ) {
			...
			if ( n % ratio == 0 ) {
				...
				for ( common = 0; common < minLength; common++ )
					if ( a[ 0 ][ common ] != a[ 1 ][ common ] ) break;
				length -= common;
	...
	/*
	 * The following (rather messy) methods implements the encoding of arbitrary integers inside a big array. Unfortunately, we have to specify different codes for almost every type. */
	 ...
	 * @param offset an offset into <code>a</code> where elements will be store.
	 * @param length a maximum number of elements to store in <code>a</code>.
	 * @return if <code>a</code> can hold the extracted elements, the number of extracted elements; otherwise, the number of remaining elements with the sign changed. */

 ...
 *
 * <P>The iterators provided by this class are type-specific {@link it.unimi.dsi.fastutil.BidirectionalIterator bidirectional iterators}. Moreover, the iterator returned by <code>iterator()</code> can
...
public class IntRBTreeSet extends AbstractIntSortedSet implements java.io.Serializable, Cloneable, IntSortedSet {
	 ...
	 *
	 * <P>When a specific {@link Comparator} is specified and stored in {@link #storedComparator}, we must check whether it is type-specific. If it is so, we can used directly, and we store it in
	...
	public IntRBTreeSet( final IntegerIterator i ) {
		while ( i.hasNext() )
			add( i.nextInt() );
	}
	...
	/* The following methods implements some basic building blocks used by all accessors. They are (and should be maintained) identical to those used in RBTreeMap.drv.
	 * 
	 ...
	 *
	 * <P>This method uses the {@link #actualComparator} if it is non-<code>null</code>. Otherwise, it resorts to primitive type comparisons or to {@link Comparable#compareTo(Object) compareTo()}.
	 *
	...
	/** This vector remembers the path and the direction followed during the current insertion. It suffices for about 2<sup>32</sup> entries. */
	 ...
	 *
	 * <P>We use the only "metadata", i.e., {@link Entry#info}, to store information about color, predecessor status and successor status.
	 *
	 * <P>Note that since the class is recursive, it can be considered equivalently a tree. */
	private static final class Entry implements Cloneable {
		 ...
		 * left.prettyPrint(level +1 ); for (int i = 0; i < level; i++) System.err.print("  "); System.err.println(key + " (" + (black() ? "black" : "red") + ")"); if ( succ() ) { for (int i = 0; i <
		 * level; i++) System.err.print("  "); System.err.println("succ: " + right ); } else if (right != null) right.prettyPrint(level + 1); } */
	}
	 ...
	 *
	 * <P>This class represents a subset. One has to specify the left/right limits (which can be set to -&infin; or &infin;). Since the subset is a view on the set, at a given moment it could happen
	 * that the limits of the range are not any longer in the main set. Thus, things such as {@link java.util.SortedSet#first()} or {@link java.util.Collection#size()} must be always computed
	 * on-the-fly. */
	private final class Subset extends AbstractIntSortedSet implements java.io.Serializable, IntSortedSet {
		 ...
		 * 
		 * <P>This class inherits from {@link SetIterator}, but overrides the methods that update the pointer after a {@link java.util.ListIterator#next()} or {@link java.util.ListIterator#previous()}
		 * . If we would move out of the range of the subset we just overwrite the next or previous entry with <code>null</code>. */
	 ...
	 *
	 * <P>This method performs a deep copy of this tree set; the data stored in the set, however, is not cloned. Note that this makes a difference only for object keys.
	 *

 ...
 *
 * <P>A semi-indirect heap is based on a <em>reference array</em>. Elements of a semi-indirect heap are integers that index the reference array (note that in an <em>indirect</em> heap you can also map
...
public class IntSemiIndirectHeaps {
	 ...
	 *
	 * <p>The <em>front</em> of a semi-indirect heap is the set of indices whose associated elements in the reference array are equal to the element associated to the first index.
	 *
	 * <p>In several circumstances you need to know the front, and scanning linearly the entire heap is not the best strategy. This method simulates (using a partial linear scan) a breadth-first visit
	 * that terminates when all visited nodes are larger than the element associated to the top index, which implies that no elements of the front can be found later. In most cases this trick yields a

 ...
 *
 * <P>Instances of this class use as reference list a <em>reference array</em>, which must be provided to each constructor. The priority queue is represented using a heap. The heap is enlarged as
 * needed, but it is never shrunk. Use the {@link #trim()} method to reduce its size, if necessary.
 *
 * <P>This implementation allows one to enqueue several time the same index, but you must be careful when calling {@link #changed()}. */
public class IntHeapSemiIndirectPriorityQueue extends AbstractIndirectPriorityQueue<Integer> implements IntIndirectPriorityQueue {
	 ...
	 *
	 * <P>The queue returned by this method will be backed by the given array. The first <code>size</code> element of the array will be rearranged so to form a heap (this is more efficient than
	 * enqueing the elements of <code>a</code> one by one).
	 *
	 * @param refArray the reference array.
	...
	/** Wraps a given array in a queue using a given comparator.
	 *
	 * <P>The queue returned by this method will be backed by the given array. The elements of the array will be rearranged so to form a heap (this is more efficient than enqueing the elements of
	 * <code>a</code> one by one).
	 *
	 * @param refArray the reference array.
	 * @param a an array of indices into <code>refArray</code>.
	 * @param c the comparator used in this queue, or <code>null</code> for the natural order. */
	...
	/** {@inheritDoc}
	 *
	 * <P>The caller <strong>must</strong> guarantee that when this method is called the index of the first element appears just once in the queue. Failure to do so will bring the queue in an

...
public class ByteBigArrays {
	 ...
	 *
	 * <P>If possible (i.e., <code>from</code> is 0) this method uses a backward loop. In this case, it is significantly faster than the corresponding method in {@link Arrays}.
	 *
	 * @param array a big array.
	...
	private static void selectionSort( final byte[][] a, final long from, final long to, final ByteComparator comp ) {
		for ( long i = from; i < to - 1; i++ ) {
			...
			for ( long j = i + 1; j < to; j++ )
				if ( comp.compare( ByteBigArrays.get( a, j ), ByteBigArrays.get( a, m ) ) < 0 ) m = j;
			if ( m != i ) swap( a, i, m );
		}
	}
	...
	private static void selectionSort( final byte[][] a, final long from, final long to ) {
		for ( long i = from; i < to - 1; i++ ) {
			long m = i;
			for ( long j = i + 1; j < to; j++ )
				if ( ( ( ByteBigArrays.get( a, j ) ) < ( ByteBigArrays.get( a, m ) ) ) ) m = j;
			if ( m != i ) swap( a, i, m );
		}
	}
	...
	public static void radixSort( final byte[][] a, final long from, final long to ) {
		...
		while ( offsetPos > 0 ) {
			...
			for ( long i = length; i-- != 0; )
				ByteBigArrays.set( digit, i, (byte)( ( ( ( ByteBigArrays.get( a, first + i ) ) >>> shift ) & DIGIT_MASK ) ^ signMask ) );
			for ( long i = length; i-- != 0; )
	...
	private static void selectionSort( final byte[][] a, final byte[][] b, final long from, final long to ) {
		for ( long i = from; i < to - 1; i++ ) {
			long m = i;
			for ( long j = i + 1; j < to; j++ )
				if ( ( ( ByteBigArrays.get( a, j ) ) < ( ByteBigArrays.get( a, m ) ) ) || ( ( ByteBigArrays.get( a, j ) ) == ( ByteBigArrays.get( a, m ) ) )
						&& ( ( ByteBigArrays.get( b, j ) ) < ( ByteBigArrays.get( b, m ) ) ) ) m = j;
			if ( m != i ) {
	...
	public static void radixSort( final byte[][] a, final byte[][] b, final long from, final long to ) {
		...
		while ( offsetPos > 0 ) {
			...
			// Count keys.
			for ( long i = length; i-- != 0; )
				ByteBigArrays.set( digit, i, (byte)( ( ( ( ByteBigArrays.get( k, first + i ) ) >>> shift ) & DIGIT_MASK ) ^ signMask ) );
			for ( long i = length; i-- != 0; )
				count[ ByteBigArrays.get( digit, i ) & 0xFF ]++;
			// Compute cumulative distribution and push non-singleton keys on stack.
			int lastUsed = -1;

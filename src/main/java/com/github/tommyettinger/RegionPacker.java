...
public class RegionPacker {
    ...
    public Region pack(char[][] map, char yes)
    {
        ...
        for(int i = 0, ml = 0; i < limit && ml < mapLimit; i++, skip++)
        {
            ...
            ml++;
            current = map[(int)pt[0]][(int)pt[1]] == yes;
            if(current != on)
            {
                packing.add((short) skip);
                skip = 0;
     ...
     * @param bounds the dimensions of the.
     * @return a boolean[][] storing which cells encoded by packed are on (true) or off (false).
     */
    public boolean[] unpack(Region packed, final long[] bounds)
    {
        if(packed == null)
        ...
        if(packed.size == 0)
            return unpacked;
        packed.forEachAlternating(new LongBooleanProcedure() {
            long idx = 0;
            public void value(long each, boolean odd) {
                if(odd)
                {
                    for (long toSkip = idx + each; idx < toSkip && idx < curve.maxDistance; idx++) {
                        long[] pt = curve.point(idx);
                        int u = 0;
